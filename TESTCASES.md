...
| 1 | Установить заголовок `Content-Type: application/json` | Заголовок установлен |
| 2 | Установить заголовок `Accept: application/json` | Заголовок установлен |
| 3 | Отправить POST /api/1/item с валидным телом | 200 OK, объявление создано, получен id1 |
| 4 | Отправить **точно такой же** POST запрос ещё раз | 200 OK, объявление создано, получен id2 (id2 ≠ id1) |

**Примечание:** API не является идемпотентным для POST-запросов. Это ожидаемое поведение для данного API, так как требования не запрещают создание дубликатов.

## Нефункциональные проверки

### TC-NF-01: Время ответа GET /api/1/item/{id}

**Предусловие:** Создано объявление через POST /api/1/item, получен его `id`

| Шаг | Действие | Ожидаемый результат |
|-----|----------|---------------------|
| 1 | Установить заголовок `Accept: application/json` | Заголовок установлен |
| 2 | Отправить GET запрос на `https://qa-internship.avito.com/api/1/item/{id}` | Время ответа не превышает 500 мс |

---

### TC-NF-02: Время ответа GET /api/1/statistic/{id}
...
|-----|----------|---------------------|
| 1 | Установить заголовок `Accept: application/json` | Заголовок установлен |
| 2 | Отправить GET запрос на `https://qa-internship.avito.com/api/1/statistic/{id}` | Время ответа не превышает 500 мс |

---

### TC-NF-03: Время ответа POST /api/1/item

| Шаг | Действие | Ожидаемый результат |
|-----|----------|---------------------|
| 1 | Установить заголовок `Content-Type: application/json` | Заголовок установлен |
| 2 | Установить заголовок `Accept: application/json` | Заголовок установлен |
| 3 | Отправить POST запрос на `https://qa-internship.avito.com/api/1/item` с валидным телом | Время ответа не превышает 1000 мс |
